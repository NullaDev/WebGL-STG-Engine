...
import { CG_BULLET, clone } from "../../../entity/Entity";
...
export const stage_006: StageEntry = {
    ...
    init: (time_scale: number) => {
        ...
        const n = 8, l0 = 160, l1 = 120, l2 = 240;
        ...
        const adder = (dir: number, da: number, a0 = Math.PI / 2) => repeat((i0) => [
            nonblock((s: Scheduler, data: S_DATA = s.custom_fields) => [
                () => {
                    const a1 = a0 + Math.PI * 2 * i0 / n * dir;
                    const a2 = a1 + Math.PI / 2 * dir;
                    EntityPool.INSTANCE.add(data.host = new MovePoint(ss0l, cf1)
                        .simpleInit(l0 * Math.cos(a1) - l2 * Math.cos(a2), l0 * Math.sin(a1) - l2 * Math.sin(a2), v0, a2))
                },
                Math.round((l2 - l1) / v0),
                () => {
                    ...
                    repeat((i1) => [
                        repeat((i2) => [
                            () => EntityPool.INSTANCE.add(new MovePoint(ss0s, cf0)
                                .simpleInit(data.host.px, data.host.py, v1 * (2 + i2) / 4, data.dir + (i1 - 1) * da))
                        ], 3)]
                        , 3),
                    t0
